...
public class MTAGFileCreationSevice {
	...
	public void processFileCreation(List<TAGDevice> tagDeviceList) throws InvalidFileFormatException {
		try {
			 ...
			 */
			case Constants.TAGN:
				nystaTAGCreationServiceImpl.createTagFile(tagDeviceList);
				break;	
			default:
	...
	public void validateAgency(int agencyId) throws InvalidFileGenerationTypeException {
		...
		if(agencyId==Constants.NYSTA_AGENCY_ID || agencyId==Constants.NYSBA_AGENCY_ID) {
	...
	public void writeContentToFileFooter(FileWriter filewriter, String record ) {
		try {
			if(!record.equalsIgnoreCase("") && !record.equalsIgnoreCase(" ")) {
				filewriter.write(record);
				log.info("Footer to file successfully: {}",record);
			}else {
				log.info("Record is blank: {}",record);
			}
			//error recovery
