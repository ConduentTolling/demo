...
public class IagController {
	...
	private static final Logger logger = LoggerFactory.getLogger(IagController.class);

	@ApiOperation(value = "Process Acknowledgement File for IAG")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Job executed successfully", response = String.class) })

	@GetMapping("/iag_start")
	public String processAckFile() throws IOException {
		try {

			MDC.put("logFileName", "PROCESS_ACK"
