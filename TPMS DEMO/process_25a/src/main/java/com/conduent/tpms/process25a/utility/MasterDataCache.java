...
import com.conduent.tpms.process25a.dao.TAgencyDao;
import com.conduent.tpms.process25a.dao.TCodesDao;
...
*/	
	...
	public Plaza getPlazaByExtPlazaId(String extPlazaId) {
		
		Optional<Plaza> tempOptionalPlaza = plazaList.stream()
				.filter(e -> extPlazaId.equalsIgnoreCase(e.getExternPlazaId().trim()) && e.getAgencyId() == NYSTA_AGENCY_ID).findFirst();
		if (tempOptionalPlaza.isPresent()) {
			return tempOptionalPlaza.get();
	...
	public AgencyInfoVO getAgency(String agencyId) {
		...
		Optional<AgencyInfoVO> agencyDto = agencyInfoVOList.stream().filter(e -> e.getDevicePrefix().equals(agencyId.trim()))
				.findFirst();
	...
	public SystemAccountVO getParamValueByName(String paramName) {
		if (paramName == null || systemAccountList == null) {
			log.info("Param config value is null or not present..");
			return null;
		}
		Optional<SystemAccountVO> plan = systemAccountList.stream().filter(e -> e.getParamName().equalsIgnoreCase(paramName)).findFirst();
		if (plan.isPresent()) {
			log.info("Param config value is presennt get systemAccount object ==> "+plan.toString());
			return plan.get();
		}
