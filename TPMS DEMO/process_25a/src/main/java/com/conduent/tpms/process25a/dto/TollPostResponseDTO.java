...
public class TollPostResponseDTO {
	...
	/*
	 * public static ICRXQueueDto getICRXQueueDetails(TollPostResponseDTO
	 * tollPostResponseDTO, AccountTollPostDTO toll) { ICRXQueueDto icrxQueueDto =
	 * new ICRXQueueDto();
	 * 
	 * icrxQueueDto.setLaneTxId(tollPostResponseDTO.getLaneTxId());
	 * icrxQueueDto.setDepositId(tollPostResponseDTO.getDepositId()); LocalDate
	 * postedDate = LocalDate.parse(tollPostResponseDTO.getPostedDate());
	 ...
	 * log.info("If ETC_POST_STATUS having [POST] --> AWPOSTVTOL"); }else
	 * if(etcTxStatus.equals(TxStatus.TX_STATUS_PPST)) {
	 * icrxQueueDto.setTxStatus(ICRXConstants.AWPOSTITOL);
	 * log.info("If ETC_POST_STATUS having [PPST] --> AWPOSTITOL"); }
	 * 
	 * //|| etcTxStatus.equals(TxStatus.TX_STATUS_NPST)-- logic not required as NPST
	 * is a part of rejection code. String plateNumber =toll.getPlateNumber();
	 * if((TollPostResponseDTO.isInValidPlateNumber(plateNumber)) &&
	 * (!(etcTxStatus.equals(TxStatus.TX_STATUS_POST)
	 * ||etcTxStatus.equals(TxStatus.TX_STATUS_PPST) ))) {
