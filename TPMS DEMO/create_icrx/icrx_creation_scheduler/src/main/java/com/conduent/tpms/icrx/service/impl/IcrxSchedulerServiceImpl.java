...
import com.conduent.tpms.icrx.thread.IcrxFileRequestThread;
 ...
 */
@Service
public class IcrxSchedulerServiceImpl implements IcrxSchedulerService {

	private static final Logger log = LoggerFactory.getLogger(IcrxSchedulerServiceImpl.class);
	...
	public void process(String fileType) {
		...
		if (!tempAwayAgencyList.isEmpty()) {
			...
			for (int i = 0; i < tempAwayAgencyList.size(); i++) {
				tempExecutorService.execute(new IcrxFileRequestThread(configVariable,
