...
public class TQatpMappingDaoImpl implements TQatpMappingDao {
	...
	public FileDetails checkIfFileProcessedAlready(String fileName) {
		...
		log.info("Query used..{}",queryToCheckFile);
		...
		{
			log.info("No record found from t_LANE_TX_CHECKPOINT for file name: {}",fileName);
			return null;
		}
	}
	...
	public Long insertFileDetails(FileDetails fileDetails) {
		...
		paramSource = new MapSqlParameterSource();
		paramSource.addValue(IITCConstants.FILE_NAME, fileDetails.getFileName());
		paramSource.addValue(IITCConstants.FILE_TYPE, fileDetails.getFileType());
		paramSource.addValue(IITCConstants.PROCESS_NAME, fileDetails.getProcessName());
		paramSource.addValue(IITCConstants.PROCESS_ID, fileDetails.getProcessId());
		paramSource.addValue(IITCConstants.PROCESS_STATUS, fileDetails.getProcessStatus().getCode());
		paramSource.addValue(IITCConstants.TX_DATE, fileDetails.getTxDate());
		paramSource.addValue(IITCConstants.LANE_TX_ID, fileDetails.getLaneTxId());
		paramSource.addValue(IITCConstants.SERIAL_NUMBER, fileDetails.getSerialNumber());
		paramSource.addValue(IITCConstants.FILE_COUNT, fileDetails.getFileCount());
		paramSource.addValue(IITCConstants.PROCESSED_COUNT, fileDetails.getProcessedCount());
		paramSource.addValue(IITCConstants.SUCCESS_COUNT, fileDetails.getSuccessCount());
		paramSource.addValue(IITCConstants.EXCEPTION_COUNT, fileDetails.getExceptionCount());
		paramSource.addValue(IITCConstants.UPDATE_TS, Timestamp.valueOf(fileDetails.getUpdateTs()));

		namedJdbcTemplate.update(query, paramSource, keyHolder, new String[] { IITCConstants.ID });
		Map<String, Object> map = keyHolder.getKeys();
		Long id = null;
		if (null != map) {
			id = Long.parseLong(map.get(IITCConstants.ID).toString());
		}
	...
	public FileParserParameters getMappingConfigurationDetails(String fileType) {
		...
		try {
			...
			paramSource.addValue(IITCConstants.AGENCY_ID, IITCConstants.MTA_AGENCY_ID);
			paramSource.addValue(IITCConstants.FILE_FORMAT, IITCConstants.FIXED);
	...
	private void getMinMaxListValues(List<MappingInfoDto> fileNameMappingDetails) {
		for (MappingInfoDto infoRow : fileNameMappingDetails) {
			if (IITCConstants.FIXED_VALUE.equals(infoRow.getValidationType())
					|| IITCConstants.DATE.equals(infoRow.getValidationType())) {
				infoRow.setFixeddValidValue(infoRow.getValidationValue());
			} else if (IITCConstants.RANGE.equals(infoRow.getValidationType())) {
			...
			} else if (IITCConstants.LOV.equals(infoRow.getValidationType())) {
