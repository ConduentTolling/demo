...
public class IAGSeviceImpl implements IAGSevice {
	 ...
	 */
	@Override
	public void loadValidTagDetails(String startDeviceNo, String endDeviceNo, String loadType, String enableHistory) {
		try {
		...
		log.info("Getting list of active tag accounts from V_ETC_ACCOUNT table : {}",tagStatusSortedAllRecordlist);
			List<TagStatusSortedRecord> tagStatusSortedRecordList = iagDao.getETCAccountInfo(tagStatusSortedAllRecordlist);

			/**
			 * Iterate on tagStatusSortedRecordList to fetch ETC account details from FPMS
			 * API
			 */
			log.info("Iterate on tagStatusSortedRecordList to fetch ETC account details from FPMS : {}",tagStatusSortedRecordList);
			for (TagStatusSortedRecord tagStatusSortedRecord : tagStatusSortedRecordList) {
				...
				AccountApiInfoDto accountApiInfoDto = callAccountApi(String.valueOf(tagStatusSortedRecord.getEtcAccountId()));
				...
				if (accountApiInfoDto != null) {
					...
					//tagStatusSortedRecord.setFirstTollDate(accountApiInfoDto.getFirstTollTimestamp() != null? accountApiInfoDto.getFirstTollTimestamp().toLocalDate(): null);
					if (accountApiInfoDto.getAcctFinStatus() != null) {
						...
						else if (Constants.FS_LOW_BAL.equals(finStatusStr)|| Constants.FS_LOW_.equals(finStatusStr)) 
						{
						...
						{
							accountApiInfoDto.setAcctFinStatus("AUTOPAY");
						}
						tagStatusSortedRecord.setFinancialStatus(masterDataCache.getTcodeId(accountApiInfoDto.getAcctFinStatus()));
					}
				...
				buildTagStatusService.buildTagStatus(tagStatusSortedRecord);
			...
			// Insertion in T_H_TAH_ALLSORTED Table
			if (enableHistory.equals(Constants.Y) && loadType.equals(Constants.F)) {
			...
			for(TagStatusSortedRecord record :tagStatusSortedRecordList)
			{
				...
				if(recordexists != null) 
				{
					...
					if(record.getAccountNo().equals(recordexists.getAccountNo()) &&
					   ...
					   record.getPostPaidStatus() == (recordexists.getPostPaidStatus()) &&
				...
				{
					log.info("Inserting record in T_TAG_ALL_SORTED table for device no..{}",deviceno);
					insertIntoTTagAllSorted(tagStatusSortedRecordList);
				}
				...
				for (TagStatusSortedRecord allsorted : tagStatusSortedRecordList) 
				{
					...
					for(TagStatusSortedRecord sortedlist : deviceinfo) 
					{
						if(allsorted.getAccountNo().equals(sortedlist.getAccountNo()) &&
				     	   allsorted.getAccountStatus() == sortedlist.getAccountStatus() &&
						   ...
						   allsorted.getRebillPayType() == sortedlist.getRebillPayType() &&
						   ...
						   allsorted.getPostPaidStatus()== sortedlist.getPostPaidStatus() &&
						...
						else
						{
							...
							iagDao.updateTTagStatusallsorted(deviceNo,deviceinfo);
						...
						if((allsorted.getMtaTagStatus() != sortedlist.getMtaTagStatus()) ||
								...
								//((sortedlist.getMtaTagStatus() == Constants.TS_INVALID) ||(sortedlist.getMtaTagStatus() == Constants.TS_LOST_STOLEN))) 
						{
							...
							if(recordforPlazaId == true)
							{
								...
								log.info("Record for MTA device no {} got updated in T_INCR_TAG_STATUS_ALLSORTED...",deviceNo);
							}
							else
							{
						...
						if((allsorted.getRioTagStatus() != sortedlist.getRioTagStatus()) || 
								(allsorted.getAgencyId() == Constants.MTA_AGENCY_ID))// && 
								//((sortedlist.getRioTagStatus() == Constants.TS_INVALID) ||(sortedlist.getRioTagStatus() == Constants.TS_LOST_STOLEN))) 
						{
							...
							if(recordforPlazaId == true)
							{
								...
								log.info("Record for RIO device no {} got updated in T_INCR_TAG_STATUS_ALLSORTED...",deviceNo);
							}
							else
							{
								...
								(allsorted.getAgencyId() == Constants.MTA_AGENCY_ID) ) 
						{
							...
							if(recordforPlazaId == true)
							{
								...
								log.info("Record for PA device no {} got updated in T_INCR_TAG_STATUS_ALLSORTED...",deviceNo);
							}
							else
							{
	...
	private void insertIntoTTagAllSorted(List<TagStatusSortedRecord> activeTagsList) {
		iagDao.insertIntoTTagAllSorted(activeTagsList);
	}
	...
	public int  getFtagPlanInfo(TagStatusSortedRecord tagStatusSortedRecord) {
		...
		try {
			 ...
			 if(   (Integer.valueOf(tagStatusSortedRecord.getPlansStr().substring((int) (Math.floor(105 / 10) * 4 ), 44))
