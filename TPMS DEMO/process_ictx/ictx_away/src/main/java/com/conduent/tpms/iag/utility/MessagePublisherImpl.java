...
public class MessagePublisherImpl implements MessagePublisher {
	...
	public long publishMessagesList(List<TranDetail> txlist, Gson gson) 
	{
		TranDetail tempTx;
		long msgCounter = 0;
		...
		{
			tempTx=txlist.get(i);
			tempTx.setDeviceNo(tempTx.getDeviceNoFromDb() != null ? tempTx.getDeviceNoFromDb() : tempTx.getDeviceNo());
			byte[] msg = gson.toJson(tempTx).getBytes();
			logger.info("Message Published::{}",gson.toJson(tempTx));
		...
		if (msgsSize != 0) 
		{
			msgCounter = publishMessages(messages);
			if (msgCounter == msgsSize) 
			{
				logger.info("Messages Published Successfully: Expectd : {},Published : {} ", msgsSize, msgCounter);
			}
