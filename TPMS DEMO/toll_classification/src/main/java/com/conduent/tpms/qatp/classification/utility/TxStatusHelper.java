...
public class TxStatusHelper {
	...
	public Integer getTxStatus(String txnType, String txnSubType, Long etcAccountId, Long laneTxId) {
		...
		/*case "O":
			...
			break;*/
		case "O":
			txStatus = QatpClassificationConstant.QECTX;//updated QECTX to QCTOL
			break;
		case "U":
		...
		case "X":
			...
			break;
		case "P":
			txStatus = QatpClassificationConstant.QFTOL;
			break;
		case "R":
			switch (txnSubType) {
			case "L":
		...
		if(txStatus!=null) {
			...
			logger.info("Tx status:{} for laneTxId: {} and CodeId: {}", txStatus, laneTxId, tempTCode.getCodeId());
