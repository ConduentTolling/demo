...
/**
 * Utility class to publish message to Oracle Streaming Service
 * 
 * @author deepeshb
 *
 */
...
public class MessagePublisherImpl implements MessagePublisher {
	...
	@Autowired
	StreamClient streamClientHome;

	@Autowired
	StreamClient streamClientVoilation;
	 ...
	 * @Autowired
	 * 
	 * @Qualifier("streamClientVoilation") StreamClient streamClientVoilation;
	 */
	 ...
	 * 
	 * File file = ResourceUtils.getFile(configVariable.getConfigfilepath()); final
	 * String configurationFilePath = file.toString();
	 * 
	 * final ConfigFileReader.ConfigFile configFile =
	 * ConfigFileReader.parse(configurationFilePath); final
	 * AuthenticationDetailsProvider provider = new
	 * ConfigFileAuthenticationDetailsProvider(configFile); final StreamAdminClient
	 * adminClient = StreamAdminClient.builder().build(provider);
	 * 
	 * Stream stream = getStream(adminClient, streamId); streamClient =
	 * StreamClient.builder().stream(stream).build(provider);
	 * 
	 * }
	 */
	...
	private StreamClient getStreamClient(String streamName) {
		if (streamName.equalsIgnoreCase("home")) {
			return streamClientHome;
		} else {
	...
	private String getStreamId(String streamName) {
		if (streamName.equalsIgnoreCase("home")) {
			return configVariable.getHomeAgencyStreamId();
		} else {
			return configVariable.getViolationStreamId();
		}
	}
	...
	public boolean publishMessage(String laneTxId, byte[] msg, String streamId) {
		...
		PutMessagesRequest putRequest = PutMessagesRequest.builder().streamId(getStreamId(streamId))
				.putMessagesDetails(messagesDetails).build();
		
		logger.debug("Put message");
		logger.debug("Runtime Stream Id :: ==>{}", putRequest.getStreamId());
		from = LocalDateTime.now();
		
		PutMessagesResponse putResponse = getStreamClient(streamId).putMessages(putRequest);
		to = LocalDateTime.now();
	 ...
	 */
	/*
	 * public long publishMessages(List<PutMessagesDetailsEntry> messages) { long
	 * msgCounter = 0;
	 * 
	 ...
	 * PutMessagesDetails.builder().messages(messages).build(); PutMessagesRequest
	 * putRequest = PutMessagesRequest.builder().streamId(streamId)
	 * .putMessagesDetails(messagesDetails).build();
	 * 
	 * logger.debug("Put message"); from = LocalDateTime.now(); PutMessagesResponse
	 * putResponse = getStreamClient(streamId).putMessages(putRequest); to =
	 * LocalDateTime.now();
	 * 
	 * logger.debug("Put message - Completed in {} ms",
	 * ChronoUnit.MILLIS.between(from, to));
	 * 
	 ...
	 * 
	 * logger.error("Message , Error {}: {}", entry.getError(),
	 * entry.getErrorMessage()); } else { msgCounter++;
	 * logger.debug("Message  published to partition {}, offset {}.",
	 * entry.getPartition(), entry.getOffset()); } } return msgCounter;
	 * 
	 * }
	 */
