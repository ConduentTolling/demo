...
public class BuildTagStatusServiceImpl implements BuildTagStatusService {
	...
	public TagStatusSortedRecord buildTagStatus(TagStatusSortedRecord tagStatusSortedRecord) {
		...
		TagStatusSortedRecord t = buildFtagStatus(tagStatusSortedRecord);
		return t;
	}
	...
	public void buildItagStatus(TagStatusSortedRecord tagStatusSortedRecord) {
		...
		int accStatus = tagStatusSortedRecord.getAccountStatus();
		...
		if (deviceStatus == Constants.DS_ACTIVE || prevalidationFlag.equalsIgnoreCase(Constants.YES)) // changed to active as told by PB
		{ 
			if(accStatus == Constants.ACTIVE_ACCOUNT_STATUS || accStatus == Constants.RVKW) {
				if (financial_status == Constants.FS_GOOD) {
					...
					if(currentBalance<Constants.NEG_THRESHOLD) {//get it from t_process_parameter 
						tagStatusSortedRecord.setItagTagStatus(Constants.TS_NEG_VAL);
					}
				} else if (financial_status == Constants.FS_ZERO || accStatus == Constants.RVKW) // add account status =																							// RVKW (PB)
				{
					tagStatusSortedRecord.setItagTagStatus(Constants.TS_NEG_VAL);
				} else {
	...
	public TagStatusSortedRecord buildFtagStatus(TagStatusSortedRecord tagStatusSortedRecord) {
		...
		int RebillPayType = tagStatusSortedRecord.getRebillPayType();
		...
		if (tagStatusSortedRecord.getOptInOutFlag().equals(Constants.IN)) {
			...
			if ((tagStatusSortedRecord.getItagTagStatus() != Constants.TS_INVALID)
				  || (RebillPayType == Constants.RPT_VISA || RebillPayType == Constants.RPT_AMEX
							|| RebillPayType == Constants.RPT_DISCOVER
							|| RebillPayType == Constants.RPT_MASTERCARD
							|| RebillPayType == Constants.RPT_ACHCPPT
							|| RebillPayType == Constants.RPT_ACHSPPT)) 
				
				//RebillPayType == Constants.RPT_DISC // remved
			{
				...
				if ((Integer
					...
					tagStatusSortedRecord.setFtagTagStatus(Constants.TS_NEG_VAL);
				} else
				{
 					tagStatusSortedRecord.setFtagTagStatus(Constants.TS_INVALID);
				}

			} else if ((RebillPayType != Constants.RPT_VISA || RebillPayType != Constants.RPT_AMEX
							|| RebillPayType != Constants.RPT_DISCOVER
							|| RebillPayType != Constants.RPT_MASTERCARD
							|| RebillPayType != Constants.RPT_ACHCPPT
							|| RebillPayType != Constants.RPT_ACHSPPT)
					&& tagStatusSortedRecord.getFtagTagStatus() != Constants.ZERO || (rvkw_status != Constants.RVKW) ) {
				//tagStatusSortedRecord.getFtagTagStatus() != Constants.ZERO
				
				if(tagStatusSortedRecord.getPlansStr()!=null) {
                    
					
					System.out.println("Plan String at line 345 is:"+ tagStatusSortedRecord.getPlansStr());
			...
			if (tagStatusSortedRecord.getOptInOutFlag().equals(Constants.OUT) && p_device_status == Constants.DS_ACTIVE && tagStatusSortedRecord.getPlansStr()!=null) {

				if (tagStatusSortedRecord.getPlansStr()!=null && (Integer
						.valueOf(tagStatusSortedRecord.getPlansStr().substring((int) (Math.floor(105 / 10) * 4 ), 44))
						& (int) Math.pow(2, Math.floorMod(105, 10))) > 0
						|| (Integer.valueOf(
								tagStatusSortedRecord.getPlansStr().substring((int) (Math.floor(104 / 10) * 4 ), 44))
			...
			} else if(tagStatusSortedRecord.getOptInOutFlag().equals(Constants.OUT) ){
				tagStatusSortedRecord.setFtagTagStatus(Constants.TS_INVALID);
			}
			

			log.info("FTAG status set as: {}", tagStatusSortedRecord.getFtagTagStatus());
			return tagStatusSortedRecord;
