...
//		{
			plaza=masterDataCache.getPlaza(etcExitPlaza,Convertor.toInteger(QATPConstants.AGENCY_ID_FNTX));
...
//		obj.setAtpFileId(atpFileId);
		...
		else if(etcExitDate!=null && validate(etcExitDate))
		{
			...
			String temp = Constants.DEFAULT_DATE + Constants.DEFAULT_TIME;
			obj.setTxTimeStamp(temp);
		}
		...
		{
			plaza = masterDataCache.getPlaza(etcEntryPlaza,Convertor.toInteger(QATPConstants.AGENCY_ID_FNTX));  
			if(plaza!=null)
			{
		...
		if(!etcTrxType.equals("P") && fileType.equals(FOConstants.FNTX))
		{
			obj.setTrxType("R");
			obj.setTrxSubtype("C");
		}
		...
		if(!etcTrxType.equals("P") && !etcTrxType.equals("N") && fileType.equals(INTXConstants.INTX))
		{
			obj.setTrxType("R");
			obj.setTrxSubtype("C");
		}
		...
		{
			obj.setFacAgency(Convertor.toInteger(etcFacAgency));
		}
		else
		{
			obj.setTrxType("R");
	...
	private LocalDateTime validDate(String temp) 
	{
		...
		private String getAetFlag(Transaction obj, MasterDataCache masterDataCache) {
			...
			Plaza plaza = masterDataCache.getPlaza(obj.getExternPlazaId(),Convertor.toInteger(QATPConstants.AGENCY_ID_FNTX));
			if(plaza != null)
			{
