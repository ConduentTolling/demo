...
//		fileDetails.setTrailerSize(trailerMappingDetails.get(trailerMappingDetails.size()-1).getEndPosition());
	...
	public void updateRecordCount(ReconciliationCheckPoint reconciliationCheckPoint) 
	{
		...
		paramSource.addValue(FOConstants.FILE_NAME, reconciliationCheckPoint.getFileName());
		paramSource.addValue(FOConstants.RECORD_COUNT, reconciliationCheckPoint.getRecordCount());
		paramSource.addValue(FOConstants.FILE_STATUS_IND,reconciliationCheckPoint.getFileStatusInd().getCode()); 
		paramSource.addValue(FOConstants.LAST_LANE_TX_ID,reconciliationCheckPoint.getLastLaneTxTd()); 
		paramSource.addValue(FOConstants.LAST_TX_TIMESTAMP,reconciliationCheckPoint.getLastTxTimeStatmp()); 
		paramSource.addValue(FOConstants.LAST_EXTERN_LANE_ID,reconciliationCheckPoint.getLastExternLaneId()); 
		paramSource.addValue(FOConstants.LAST_EXTERN_PLAZA_ID,reconciliationCheckPoint.getLastExternPlazaId());
		paramSource.addValue(FOConstants.ATRN_FILE_NUM,reconciliationCheckPoint.getFileSeqNum());
		paramSource.addValue(FOConstants.FILE_TYPE,reconciliationCheckPoint.getFileType());
		paramSource.addValue(FOConstants.XFER_CONTROL_ID, reconciliationCheckPoint.getXferControlId());
	...
	public boolean checkForDuplicateFileNum(String value,String fileType) 
	{
		...
		paramSource.addValue(FOConstants.ATRN_FILE_NUM, value);
		paramSource.addValue(FOConstants.FILE_TYPE, fileType);
	...
	{
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		paramSource.addValue(fileType, Constants.FILE_TYPE);
