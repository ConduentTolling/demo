...
import com.conduent.tpms.iag.exception.InvalidAgencyIdException;
...
import com.conduent.tpms.iag.service.impl.ITGUFileCreationServiceImpl;
...
public class IAGFileCreationSevice {
	...
	@Autowired
	public ITGUFileCreationServiceImpl ITGUFilcereationserviceImpl;
	...
	public String fileCreationTemplate(int agencyId, String fileType, String genType) throws Exception {
		try {
			validateFileEntities(genType, fileType, agencyId);
			initialize();
	...
	public boolean processFileCreation(List<TAGDevice> info)throws InvalidFileGenerationTypeException, InvalidFileFormatException {
		try {
			...
			case Constants.ITGU :
				isFileCreated = ITGUFilcereationserviceImpl.createITGUFile(info);
				break;
	...
	public void validateFileEntities(String genType, String fileType, int agencyId) throws InvalidFileGenerationTypeException, InvalidFileFormatException, InvalidAgencyIdException {
		...
		if(fileType.equals(Constants.ITGU)) {
			log.info("Valid file type: "+fileType);
		}else {
			throw new InvalidFileFormatException("Invalid file type: "+fileType);
		}
		
		if(agencyId == 2) {
			log.info("Valid agencyId: "+agencyId);
		} else {
			throw new InvalidAgencyIdException("Invalid Agency Id: " +agencyId);
		}
	}
