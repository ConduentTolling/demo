...
public class MasterCache 
{
	...
	public Long getSystemAccount(Integer agencyId)
	{
		return systemAccount.get(agencyId);
	}
	...
	public RevenueDateStatisticsDto getRevenueStatusByPlaza(Integer plazaId) {
		...
		if (tempRevenueDateStatisticsDto != null) {
			...
			if (tempOptionalPlaza.isPresent()) {
				...
				tempRevenueDateStatisticsDto.setRevenueTime(tempPlaza.getRevenueTime()); //rev time of plaza
	...
	//	logger.info("systemAccount present {}",systemAccount.get("REJ_SYSTEM_ACCOUNT"));
		
		LocalDateTime fromTime7 = LocalDateTime.now();
		agencyHolidayList=agencyHolidayService.getAgencyHoliday();
		logger.debug("##M-SQL Time Taken for thread {} HOSTNAME: {} in agencyHolidayList: {} ms", Thread.currentThread().getName(),System.getenv("HOSTNAME"), ChronoUnit.MILLIS.between(fromTime7, LocalDateTime.now()));
