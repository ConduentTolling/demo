...
public class InsertFOTrnxStatDaoImpl implements InsertFOTrnxStatDao {
	...
	public void insertFTXNData(XferControl xferControl,String fileType1,long recordCount) {
		...
		if (!(qatpStatisticsList.hasNext())) {
			...
			for (String fileType : fileTypeList) {
				...
				jdbcTemplate.update(query, sqlAtpId, fileType,
						xferControl.getXferFileName(), LocalDate.now(), LocalTime.now().toString(), CorrConstants.ZERO,
						CorrConstants.ZERO, CorrConstants.NO, LocalDate.now(), LocalTime.now().toString(),
						CorrConstants.ZERO, updateTS, xferContrId);
	...
	public void insertITXNData(XferControl xferControl,String fileType1,long recordCount) {
		// duplicate checking from t_qatp_statistics table
		String queryForSearch1 = LoadJpaQueries.getQueryById("GET_T_QATP_STATISTICS");
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		paramSource.addValue("FILE_NAME", xferControl.getXferFileName());
	...
	{
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		paramSource.addValue(CorrConstants.FILE_CONTROL_ID, xferControlId);
		paramSource.addValue(CorrConstants.TRX_EPS_FILENAME, fileName);
	...
	public int updateStatisticsTable(long xferControlId,String fileName) 
	{
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		paramSource.addValue(CorrConstants.FILE_CONTROL_ID, xferControlId);
		paramSource.addValue(CorrConstants.TRX_FILE_NAME, fileName);
	...
	{
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		paramSource.addValue(CorrConstants.XFER_CONTROL_ID, xferControlId);
	...
	public int updateFOFileStats(long xferControlId, String fileName,ReconciliationCheckPoint reconciliationCheckPoint,GenericValidation genericValidation) throws InvalidFileHeaderException 
	{
		...
		paramSource.addValue(CorrConstants.FROM_AGENCY, fileName.substring(0,3));
		paramSource.addValue(CorrConstants.FILE_DATE, genericValidation.getFormattedDate(fileName.substring(8,16),"yyyyMMdd"));
		paramSource.addValue(CorrConstants.FILE_SEQ_NUM, reconciliationCheckPoint.getFileSeqNum());
		paramSource.addValue(CorrConstants.FILE_CONTROL_ID, xferControlId);
		paramSource.addValue(CorrConstants.TRX_FILE_NAME, fileName);
	...
	public int updateFOReconFileStats(long xferControlId, String fileName,ReconciliationCheckPoint reconciliationCheckPoint) 
	{
		...
		paramSource.addValue(CorrConstants.FROM_AGENCY, fileName.substring(0,3));
		paramSource.addValue(CorrConstants.FILE_SEQ_NUMBER, reconciliationCheckPoint.getFileSeqNum());
		paramSource.addValue(CorrConstants.TRX_EPS_AMOUNT, amount);
		paramSource.addValue(CorrConstants.FILE_CONTROL_ID, xferControlId);
		paramSource.addValue(CorrConstants.TRX_EPS_FILENAME, fileName);
	...
	public void insertInFileStat(String fileType, long recordCount, XferControl xferControl) 
	{
		...
		if(fileType.equals(CorrConstants.FTXN))
		{
			...
			if (!(qatpStatisticsList1.hasNext())) {
				...
				query1 = LoadJpaQueries.getQueryById("INSERT_T_FO_FILE_STATS");
					...
					jdbcTemplate.update(query1,xferContrId,fileName,null,
							CorrConstants.NO,recordCount,CorrConstants.ZERO,CorrConstants.ZERO,
							CorrConstants.FTXN_TO_AGENCY,null,null,CorrConstants.FTXN,updateTS);
			}
		}
		else if (fileType.equals(CorrConstants.ITXN))
		{
				...
				query1 = LoadJpaQueries.getQueryById("INSERT_T_FO_RECON_FILE_STATS");
					...
					// Inserting Data into TPMS.T_QATP_STATISTICS table
					 jdbcTemplate.update(query1,xferContrId,fileName,
							recordCount,CorrConstants.ZERO,CorrConstants.NO,null,null,null,null,
							CorrConstants.ZERO,CorrConstants.FTXN_TO_AGENCY,null,updateTS);
			}
		}
