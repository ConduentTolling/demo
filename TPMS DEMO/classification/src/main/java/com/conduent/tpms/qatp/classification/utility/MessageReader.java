...
public class MessageReader {
	...
	@Autowired
	StreamClient classificationClient;
	...
	public void init()  {
		...
		cursor = getCursorByGroup(classificationClient, configVariable.getStreamId(), configVariable.getGroupName(), node);
	 ...
	 * 
	 * Stream stream = getStream(adminClient, streamId); return
	...
	private void heartBeat() throws IOException {
		try {
			...
			classificationClient.consumerHeartbeat(consumerHeartbeatRequest);
		} catch (Exception ex) {
	...
	public GetMessagesResponse getMessage() throws IOException {
		...
		try {
			...
			getMessagesResponse = classificationClient.getMessages(messagesRequest);
	...
	public GetMessagesResponse getMessage(String streamId) throws IOException {
		GetMessagesResponse getMessagesResponse = null;
		try {
			if (classificationClient == null) {
				init();
