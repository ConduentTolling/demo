...
public class ComputeTollDaoImpl implements ComputeTollDao {
	...
	SimpleDateFormat fromUser1 = new SimpleDateFormat("hh:mm:ss");
	SimpleDateFormat myFormat1 = new SimpleDateFormat("hhmmss");
	...
	private String getFileType(TransactionDto transactionDto) {
		...
		if((transactionDto.getTxType().equals("P") || transactionDto.getTxType().equals("C")) && transactionDto.getTxSubType().equals("Z"))
		{
			return "ATPR";
		}
		else if(transactionDto.getTxType().equals("P") && transactionDto.getTxSubType().equals("T"))
		{
			return "INTX";
		}
		else if(transactionDto.getTxType().equals("R"))
		{
			return "REJC";
		}
		else
...
//			ps.setTimestamp(8, Timestamp.valueOf(commonDTO.getTxTimestamp().toLocalDateTime()));
	...
	public void insertToTEPSINTXLog(TransactionDto tempTxDto,MasterDataCache masterDataCache) {
		...
		logger.info("Insert into T_EPS_INTX_LOG for lane tx id {}", tempTxDto.getLaneTxId());
		try {
			String query = LoadJpaQueries.getQueryById("INSERT_INTO_T_EPS_INTX_LOG");
			MapSqlParameterSource paramSource = new MapSqlParameterSource();
			
			paramSource.addValue("ETC_CORR_REASON_ID",0);
			paramSource.addValue("ETC_TRX_SERIAL_NUM",tempTxDto.getTxExternRefNo().substring(4,16));
			paramSource.addValue("ETC_REVENUE_DATE",myFormat.format(fromUser.parse(tempTxDto.getReceivedDate().toString().substring(0,10))));
			paramSource.addValue("ETC_FAC_AGENCY",tempTxDto.getFacAgency());
			paramSource.addValue("ETC_TRX_TYPE",tempTxDto.getTollSystemType());
			paramSource.addValue("ETC_ENTRY_DATE",myFormat.format(fromUser.parse(tempTxDto.getEntryTxTimeStamp().toString().substring(0,10))));
			paramSource.addValue("ETC_ENTRY_TIME",myFormat1.format(fromUser1.parse(tempTxDto.getEntryTxTimeStamp().toString().substring(11,19))));
			paramSource.addValue("ETC_ENTRY_PLAZA",masterDataCache.getPlazaFromValue(tempTxDto.getEntryPlazaId()));
			paramSource.addValue("ETC_ENTRY_LANE",masterDataCache.getLaneFromValue(tempTxDto.getEntryLaneId()));
			paramSource.addValue("ETC_TAG_AGENCY",tempTxDto.getDeviceNo().substring(0,3));
			paramSource.addValue("ETC_TAG_SERIAL_NUMBER",tempTxDto.getDeviceNo().substring(4,11));
			paramSource.addValue("ETC_READ_PERFORMANCE",tempTxDto.getReadPerf());
			paramSource.addValue("ETC_WRITE_PERF",tempTxDto.getWritePerf());
			paramSource.addValue("ETC_TAG_PGM_STATUS",tempTxDto.getProgStat());
			paramSource.addValue("ETC_LANE_MODE",tempTxDto.getLaneMode());
			paramSource.addValue("ETC_VALIDATION_STATUS",tempTxDto.getEtcValidStatus());
			paramSource.addValue("ETC_LIC_STATE",null);
			paramSource.addValue("ETC_LIC_NUMBER",null);
			paramSource.addValue("ETC_CLASS_CHARGED",null);
			paramSource.addValue("ETC_ACTUAL_AXLES",tempTxDto.getActualAxles());
			paramSource.addValue("ETC_EXIT_SPEED",tempTxDto.getSpeedViolation());
			paramSource.addValue("ETC_OVER_SPEED",tempTxDto.getSpeedViolation());
			paramSource.addValue("ETC_EXIT_DATE",myFormat.format(fromUser.parse(tempTxDto.getTxTimeStamp().toString().substring(0,10))));
			paramSource.addValue("ETC_EXIT_TIME",myFormat1.format(fromUser1.parse(tempTxDto.getTxTimeStamp().toString().substring(11,19))));
			paramSource.addValue("ETC_EXIT_PLAZA",masterDataCache.getPlazaFromValue(tempTxDto.getPlazaId()));
			paramSource.addValue("ETC_EXIT_LANE",masterDataCache.getLaneFromValue(tempTxDto.getLaneId()));
			paramSource.addValue("ETC_DEBIT_CREDIT",tempTxDto.getDebitCredit());
			paramSource.addValue("ETC_AMOUNT_DUE",tempTxDto.getPostedFareAmount());
			paramSource.addValue("LANE_TX_ID",tempTxDto.getLaneTxId());
			paramSource.addValue("ATP_FILE_ID",tempTxDto.getAtpFileId());
			paramSource.addValue("TX_TYPE",tempTxDto.getTxType());
			paramSource.addValue("TX_SUB_TYPE",tempTxDto.getTxSubType()); 
			paramSource.addValue("TX_STATUS",tempTxDto.getTxStatus());
			paramSource.addValue("EXTERN_FILE_ID",tempTxDto.getExtFileId());
			paramSource.addValue("ETC_ENTRY_TIMESTAMP",tempTxDto.getEntryTxTimeStamp());
			paramSource.addValue("ETC_TX_TIMESTAMP",tempTxDto.getTxTimeStamp());
			
			namedJdbcTemplate.update(query, paramSource);
			logger.info("Successfully data inserted into T_EPS_INTX_LOG Table for Lane_TX_Id {}",tempTxDto.getLaneTxId());
		} catch (Exception ex) {
			logger.info("Record not inserted into T_EPS_INTX_LOG Table properly {}" + ex.getMessage());
		}
	...
	public void insertToTEPSFTOLLog(TransactionDto tempTxDto) {
		...
		try 
		{
			String query = LoadJpaQueries.getQueryById("INSERT_INTO_T_EPS_FTOL_LOG");
			MapSqlParameterSource paramSource = new MapSqlParameterSource();
			
			paramSource.addValue("RECORD_TYPE",null);
			paramSource.addValue("LANE_TX_ID",tempTxDto.getLaneTxId());			
			paramSource.addValue("TRANSACTION_TYPE",tempTxDto.getTxType());
			paramSource.addValue("TRANSACTION_SUB_TYPE",tempTxDto.getTxSubType());
			paramSource.addValue("TOLL_SYSTEM_TYPE",tempTxDto.getTollSystemType());
			paramSource.addValue("TOLL_REVENUE_TYPE",tempTxDto.getTollRevenueType());
			paramSource.addValue("ACCOUNT_TYPE",tempTxDto.getAccountType());
			paramSource.addValue("ENTRY_DATE",myFormat.format(fromUser.parse(tempTxDto.getEntryTxTimeStamp().toString().substring(0,10))));
			paramSource.addValue("ENTRY_TIME",myFormat1.format(fromUser1.parse(tempTxDto.getEntryTxTimeStamp().toString().substring(11,19))));
			paramSource.addValue("ENTRY_LANE_ID",tempTxDto.getLaneId());
			paramSource.addValue("ENTRY_LANE_SERIAL",tempTxDto.getEntryTxSeqNumber());
			paramSource.addValue("ENTRY_DATA_SOURCE",tempTxDto.getEntryDataSource());
			paramSource.addValue("ENTRY_VEHICLE_SPEED",tempTxDto.getEntryVehicleSpeed());
			paramSource.addValue("DEVICE_AGENCY_ID",null);
			paramSource.addValue("DEVICE_SERIAL_NUMBER",null);
			paramSource.addValue("LANE_TYPE",null);
			paramSource.addValue("LANE_MODE",tempTxDto.getLaneMode());
			paramSource.addValue("LANE_STATE",tempTxDto.getLaneState());
			paramSource.addValue("COLLECTOR_ID",tempTxDto.getCollectorNumber());
			paramSource.addValue("ACTUAL_CLASS",tempTxDto.getActualClass());
			paramSource.addValue("ACTUAL_AXLES",tempTxDto.getActualAxles());
			paramSource.addValue("ACTUAL_EXTRA_AXLES",tempTxDto.getExtraAxles());
			paramSource.addValue("TAG_CLASS",tempTxDto.getTagClass());
			paramSource.addValue("TAG_AXLES",tempTxDto.getTagAxles());
			paramSource.addValue("TAG_IAG_CLASS",tempTxDto.getTagIagClass());
			paramSource.addValue("AVC_POST_CLASS",tempTxDto.getAvcClass());
			paramSource.addValue("AVC_POST_AXLES",tempTxDto.getAvcAxles());
			paramSource.addValue("ETC_VALID_STATUS",tempTxDto.getEtcValidStatus());
			paramSource.addValue("DEVICE_READ_PERF",tempTxDto.getReadPerf());
			paramSource.addValue("DEVICE_WRITE_PERF",tempTxDto.getWritePerf());
			paramSource.addValue("DEVICE_PGM_STATUS",tempTxDto.getProgStat());
			paramSource.addValue("IMAGE_CAPTURED",tempTxDto.getImageCaptured());
			paramSource.addValue("VEHICLE_SPEED",tempTxDto.getVehicleSpeed());
			paramSource.addValue("SPEED_VIOLATION",tempTxDto.getSpeedViolation());
			paramSource.addValue("RECIPROCITY_TXN",tempTxDto.getReciprocityTrx());
			paramSource.addValue("TRANS_DATE",myFormat.format(fromUser.parse(tempTxDto.getTxTimeStamp().toString().substring(0,10))));
			paramSource.addValue("TRANS_TIME",myFormat1.format(fromUser1.parse(tempTxDto.getTxTimeStamp().toString().substring(11,19))));			
			paramSource.addValue("TRANS_LANE_ID",null);
			paramSource.addValue("TRANS_LANE_SERIAL",null);
			paramSource.addValue("BUFFERED_READ_IND",tempTxDto.getBufRead());
			paramSource.addValue("ETC_ACCOUNT_ID",tempTxDto.getEtcAccountId());
			paramSource.addValue("SUB_ACCOUNT_ID",tempTxDto.getEtcSubAccount());
			paramSource.addValue("DEBIT_CREDIT_IND",tempTxDto.getDebitCredit());
			paramSource.addValue("TOLL_AMOUNT",tempTxDto.getPostedFareAmount());
			paramSource.addValue("LICENSE_NUMBER",null);
			paramSource.addValue("LICENSE_STATE",null);
			paramSource.addValue("LANE_HEALTH",tempTxDto.getLaneHealth());
			paramSource.addValue("READER_ID",tempTxDto.getReaderId());
			paramSource.addValue("EXT_HOST_REF_NUMBER",tempTxDto.getTxExternRefNo());
			paramSource.addValue("EXTERNAL_FILE_ID",tempTxDto.getExtFileId());
			
			namedJdbcTemplate.update(query, paramSource);
			logger.info("Successfully data inserted into T_EPS_FTOL_LOG Table for Lane_TX_Id {}",tempTxDto.getLaneTxId());
		} catch (Exception e) {
			logger.info("Record not inserted into T_EPS_FTOL_LOG Table properly {}",e.getMessage());
		}
	}
}
