...
@Service
@Primary
public class FTAGFullLoadServiceImpl implements FTAGFullLoadService{

	private static final Logger log = LoggerFactory.getLogger(FTAGFullLoadServiceImpl.class);
	...
	@Autowired
	FTAGFullLoadServiceImpl ftagFullLoadSevice;
	...
	public boolean createFtagFile(int agencyId, String fileFormat, String genType) {
		...
		ftagFileDest = parentDest.concat("//").concat(Constants.FTAG_FILES); ///QaOKEFSS/QA/TOLLING/TPMS/PHASE2/IAG/PARENT_DIRECTORY/FTAG
		//FTAG_FILES
		statFileDest = parentDest.concat("//").concat(Constants.FTAG_STAT_FILES); 
		...
		try {
			tagDeviceList = iagDao.getDeviceListFromTTagAllSorted(agencyId, genType);
			...
			if (!tagDeviceList.isEmpty()) {
				...
				name.append(fileName.getFromAgencyId()).append(Constants.UNDERSCORE).append(fileName.getFileDateTime())
						.append(Constants.DOT).append(Constants.FTAG);
				log.info("File name: {}", name);

				// Setting file header DTO
				fileHeader.setFileType(Constants.FTAG);
				...
				header.append(Constants.FTAG).append(fileHeader.getFromAgencyId()).append(fileHeader.getFileDateTime())
				...
				isFileWritten = writeContentToFile(name.toString(), header.toString(), ftagFileDest,
						tagDeviceList);

				// Overwriting header of FTAG file
				if (isFileWritten) {
					overwriteFileHeader(fileHeader, file);
					summaryFileCreationService.buildSummaryFile(recordCountMap, Constants.FTAG, statFileDest,file.getName());
	...
	private boolean writeContentToFile(String fileName, String fileHeaderStr, String fileDestPath,
		...
		try (FileWriter filewriterFtag = new FileWriter(file, true)) {
			...
			for (FTAGDevice FTAGDeviceObj : tagRecords) {
				...
				if (row.length() == Constants.FTAG_REC_LENGTH && isValidRecord) {
	...
	private boolean overwriteFileHeader(FileHeaderDto fileHeaderDto, File file) {
		...
		newheader.append(Constants.FTAG).append(fileHeader.getFromAgencyId()).append(fileHeader.getFileDateTime())
				.append(fileHeader.getRecordCount()).append(fileHeader.getCountStat1())
				.append(fileHeader.getCountStat2()).append(fileHeader.getCountStat3())
				.append(fileHeader.getCountStat4());
	...
	public boolean processFileCreation(int agencyId, String fileFormat, String genType) {
		try {
			...
			if(Constants.FTAG.equals(fileFormat)) 
			{
