	...
	public long publishMessages(List<TranDetail> txlist, List<AtpFileIdObject> atpFileIdList) throws Exception
	{
		...
		for (int i = 0; i < txlist.size(); i++) 
		{
			...
			if(tempTx.getPlazaAgencyId()==null) {
				tempTx.setPlazaAgencyId(Convertor.toInteger(QATPConstants.PA_AGENCY_ID));
			}
			if(tempTx.getPlazaId()==null) {
				tempTx.setPlazaId(masterDataCache.getPlazaIfNull(Convertor.toInteger(QATPConstants.PA_AGENCY_ID)));
			}
