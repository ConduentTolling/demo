...
public class TQatpMappingDaoImpl implements TQatpMappingDao 
{
	...
	public boolean checkIfTxExternRefNoIsAlreadyUsedWithSameExtFileId(String txExternRefNo, Long extFileId)
	{
		...
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryToCheckNo = LoadJpaQueries.getQueryById("CHECK_TX_EXTERN_REF_NO_IF_USED_WITH_SAME_EXT_FILE_ID");

		paramSource.addValue(QATPConstants.TX_EXTERN_REF_NO, txExternRefNo);
		paramSource.addValue(QATPConstants.EXT_FILE_ID, extFileId);
	...
	public Long insertReconciliationCheckPoint(ReconciliationCheckPoint reconciliationCheckPoint)
	{
		...
		paramSource.addValue(QATPConstants.ATRN_FILE_NUM,reconciliationCheckPoint.getFileSeqNum());
	...
	public void updateRecordCount(ReconciliationCheckPoint reconciliationCheckPoint) 
	{
		...
		paramSource.addValue(QATPConstants.LAST_EXTERN_PLAZA_ID,reconciliationCheckPoint.getLastExternPlazaId()); 
		paramSource.addValue(QATPConstants.ATRN_FILE_NUM,reconciliationCheckPoint.getFileSeqNum());
		paramSource.addValue(Constants.FILE_TYPE,Constants.ATRN);
	...
	public boolean checkForDuplicateFileNum(String value) 
	{
		...
		paramSource.addValue(QATPConstants.ATRN_FILE_NUM, value);
	...
	{
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryToGetFileName = "SELECT FILE_SEQ_NUM FROM  (SELECT * from TPMS.T_RECONCILIATION_CHECKPOINT WHERE FILE_TYPE='ATRN' AND FILE_STATUS_IND='C' ORDER BY FILE_SEQ_NUM DESC) WHERE ROWNUM < 2";
		Integer lastSeqNum;
