...
public class PaFixlengthParser extends FileParserImpl {
	...
	public ArrayList<String> serialnumlist = new ArrayList<String>();
	Boolean dup;
	...
	@Override
	public void doFieldMapping(String value, MappingInfoDto fMapping, Long recordCount) throws InvlidFileHeaderException, ATRNFileNumDuplicationException {
		check = true;
		...
		case "D_AG_EZPASS_AMOUNT_DIS2":
			...
			break;
		case "D_AG_EZPASS_AMOUNT_DIS3":
			detailVO.setAgEzpassAmountDs3(value);
			break;
		case "D_AG_EZPASS_AMOUNT_DIS4":
			detailVO.setAgEzpassAmountDs4(value);
			break;		
	...
	public void finishFileProcess() throws CustomException 
	{
		try 
		{
			for (PaDetailInfoVO obj : detailsRecord) 
			{
				...
				try
				{
					transDetailDao.saveTransDetail(t.getTranDetail(timeZoneConv, masterDataCache, dup));								
				}
				catch(Exception ex)
				{
					ex.printStackTrace();
...
//		finally 
//		{
//			logger.info("Serial Num List : {}" ,serialnumlist.toString());
//			serialnumlist.clear();
//			logger.info("Serial Num List is Empty: {}" ,serialnumlist.toString());
//		}
	}
	@Override
		...
		if (detailsRecord != null && !detailsRecord.isEmpty() && detailsRecord.size() > 0)
		{
			...
			{		
				...
				for (PaDetailInfoVO obj : detailsRecord) 
				{
					...
					if(serialnumlist.isEmpty() || !serialnumlist.contains(obj.getAgTrxSerialNum()))
					{							
						serialnumlist.add(obj.getAgTrxSerialNum());
						dup = false;
					}
					else if (serialnumlist.contains(obj.getAgTrxSerialNum()))
					{
						dup = true;
					}
						
					tranDetail = trxn.getTranDetail(timeZoneConv, masterDataCache, dup);
...
//					
				...
				try 
				{
					transDetailService.publishMessages(list, atpFileIdList);
					check = false;
	...
	public AckFileWrapper prePareAckFileMetaData(AckStatusCode ackStatusCode, File file) 
	{
		...
		sbFileContent.append(StringUtils.rightPad("ACK", 4)).append(file.getName().substring(5, 9))//4,7
				.append(file.getName().substring(0, 4)).append(StringUtils.rightPad(file.getName(), 50))
		...
		ackFileInfoDto.setToAgency("CTH");  // according to PA NEW ICD toAgency is 0000 - file.getName().substring(5, 9)
	...
	@Override
	public void processedPartiallyExecutedFile(File file,String externFileId) throws IOException, InvalidFileNameException, InvlidFileHeaderException, EmptyLineException, InvlidFileDetailException, InterruptedException, InvalidRecordCountException, ATRNFileNumDuplicationException
	{
		...
		{
			if(currentLine.substring(0,12).matches(externFileId.toString().substring(4, 16)))
			{
				...
				for(;;nextRecord=reader.readLine())
				{
					if(nextRecord != null)
					{
						...
						isFirstRead=false;
						parseAndValidateDetails(nextRecord, 0L);
