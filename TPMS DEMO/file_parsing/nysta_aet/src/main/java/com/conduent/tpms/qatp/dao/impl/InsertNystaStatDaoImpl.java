...
import com.conduent.tpms.qatp.dao.InsertNystaStatDao;
import com.conduent.tpms.qatp.model.QatpStatistics;
import com.conduent.tpms.qatp.model.XferControl;

@Repository
public class InsertNystaStatDaoImpl implements InsertNystaStatDao {

	private static final Logger logger = LoggerFactory.getLogger(InsertNystaStatDaoImpl.class);
...
//					+ "'"+ fileName + "' order by xfer_control_id desc";
	...
	public int insertNystaData(XferControl xferControl) {
		...
		if (!(qatpStatisticsList.hasNext())) {
			...
			for (String fileType : fileTypeList) {
				...
				logger.info(sqlAtpId + " : atp file Id for file type {}", fileType);
