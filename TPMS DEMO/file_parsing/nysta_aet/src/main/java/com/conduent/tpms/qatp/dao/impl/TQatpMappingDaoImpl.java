...
public class TQatpMappingDaoImpl implements TQatpMappingDao 
{
	...
	@Override
	public FileParserParameters getMappingConfigurationDetails(FileParserParameters fielDto, Integer agencyIdNystaOrNysba) {
		...
		paramSource.addValue(Constants.AGENCY_ID, agencyIdNystaOrNysba);
	...
	@Override
	public void updateFileStatus(ReconciliationCheckPoint reconciliationCheckPoint,FileStatusInd status) 
	{
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String query = LoadJpaQueries.getQueryById("UPDATE_FILE_STATUS");
		paramSource.addValue(QATPConstants.FILE_STATUS_IND,status.getCode()); 
		paramSource.addValue(QATPConstants.FILE_NAME, reconciliationCheckPoint.getFileName());
		namedJdbcTemplate.update(query, paramSource);
	}
