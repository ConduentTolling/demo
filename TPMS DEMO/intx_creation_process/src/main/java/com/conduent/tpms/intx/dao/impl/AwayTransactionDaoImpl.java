...
public class AwayTransactionDaoImpl implements AwayTransactionDao {
	...
	/**
	 * List of Away Transaction based on externalFileId and awayAgencyId
	 * 
	 ...
	 */
	@Override
	public List<AwayTransaction> getAwayTransactionByExternalIdAndDevicePrefix(String externalFileId,
			String devicePrefix) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IntxConstant.GET_T_AWAY_TRANSACTION);
		paramSource.addValue(IntxConstant.EXTERNAL_FILE_ID, externalFileId);
		paramSource.addValue(IntxConstant.DEVICE_PREFIX, devicePrefix + "%");
		List<AwayTransaction> tempAwayTxList = namedParameterJdbcTemplate.query(queryFile, paramSource, BeanPropertyRowMapper.newInstance(AwayTransaction.class));
		if (tempAwayTxList != null) {
			return tempAwayTxList;
		}
		return new ArrayList<>();
	}
	
	/**
	 * List of Correction Away Transaction based on externalFileId and awayAgencyId
	 * 
	 * @param externalFileId
	 * @param awayAgencyId
	 * @return List<AwayTransaction>
	 */
	@Override
	public List<AwayTransaction> getCorrAwayTransactionByExternalIdAndDevicePrefix(String externalFileId,
			String devicePrefix) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IntxConstant.GET_T_CORR_AWAY_TRANSACTION);
		paramSource.addValue(IntxConstant.EXTERNAL_FILE_ID, externalFileId);
		paramSource.addValue(IntxConstant.DEVICE_PREFIX, devicePrefix + "%");
		List<AwayTransaction> tempAwayTxList = namedParameterJdbcTemplate.query(queryFile, paramSource, BeanPropertyRowMapper.newInstance(AwayTransaction.class));
		if (tempAwayTxList != null) {
	 ...
	 */
	@Override
	public List<AwayTransaction> getAwayTransactionByDevicePrefix(String devicePrefix) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IntxConstant.GET_T_AWAY_TRANSACTION);
		paramSource.addValue(IntxConstant.DEVICE_PREFIX, devicePrefix + "%");
		List<AwayTransaction> tempAwayTxList = namedParameterJdbcTemplate.query(queryFile, paramSource, BeanPropertyRowMapper.newInstance(AwayTransaction.class));
	 ...
	 */
	@Override
	public List<AwayTransaction> getCorrAwayTransactionByDevicePrefix(String devicePrefix) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IntxConstant.GET_T_CORR_AWAY_TRANSACTION);
		paramSource.addValue(IntxConstant.DEVICE_PREFIX, devicePrefix + "%");
		//System.out.println("queryFile "+ queryFile);
		List<AwayTransaction> tempAwayTxList = namedParameterJdbcTemplate.query(queryFile, paramSource,
	 ...
	 */
	@Override
	public void updateDstAtpFileId(List<AwayTransaction> awayTransactionList, Long atpFileId) {
		try {
			logger.info("Updating atpFileId {}",atpFileId);
			String query = LoadJpaQueries.getQueryById(IntxConstant.UPDATE_DST_ATP_FILE_ID);
			jdbcTemplate.batchUpdate(query, new BatchPreparedStatementSetter() {
				@Override
				public void setValues(PreparedStatement ps, int i) throws SQLException {
					AwayTransaction awayTransaction = awayTransactionList.get(i);
					ps.setLong(1, atpFileId);
					ps.setLong(2, awayTransaction.getLaneTxId());
				}

				@Override
				public int getBatchSize() {
					return awayTransactionList.size();
				}
			});
		} catch (DataAccessException e) {
			logger.info("Exception in updateDstAtpFileId..",e.getMessage());
			e.printStackTrace();
		}
	 ...
	 */
	@Override
	public void updateExpectedRevenueAmtAndRevenueDate(AwayTransaction awayTransaction) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IntxConstant.UPDATE_EXPECTED_REVENUE_AMOUNT_AND_REVENUE_DATE);
		paramSource.addValue(IntxConstant.QUERY_PARAM_EXPECTED_REVENUE_AMOUNT, awayTransaction.getExpectedRevenueAmount());
		paramSource.addValue(IntxConstant.QUERY_PARAM_REVENUE_DATE, awayTransaction.getRevenueDate());
		paramSource.addValue(IntxConstant.QUERY_PARAM_LANE_TX_ID, awayTransaction.getLaneTxId());
	 ...
	 */
	@Override
	public Integer deleteRejectedTx(Long laneTxId, String fileType) {
		
		String query = "";
		
		query = LoadJpaQueries.getQueryById(IntxConstant.DELETE_REJECTED_TX);
		
		Object[] params = { laneTxId };
	 ...
	 */
	@Override
	public Integer deleteRejectedITXNQ(Long laneTxId, String fileType) {
		
		String query = "";
		
		query = LoadJpaQueries.getQueryById(IntxConstant.DELETE_REJECTED_CORR);
	 ...
	 */
	@Override
	public Optional<List<AwayTransaction>> getAwayTransaction(String deviceNo, Integer plazaId, Integer laneId,
			LocalDateTime fromDate, LocalDateTime toDate) {
		String fromDateStr = localDateTimeUtility.getDateTimeByFormat(fromDate, IntxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String toDateStr = localDateTimeUtility.getDateTimeByFormat(toDate, IntxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String query = LoadJpaQueries.getQueryById(IntxConstant.GET_T_AWAY_TRANSACTION_DUPLICATE_CHECK);
		List<AwayTransaction> list = jdbcTemplate.query(query,
				new BeanPropertyRowMapper<AwayTransaction>(AwayTransaction.class), deviceNo, plazaId, laneId,
	 ...
	 */
	@Override
	public Optional<List<AwayTransaction>> getAwayTransactionFpms(String deviceNo, Integer plazaId, Integer laneId,
			LocalDateTime fromDate, LocalDateTime toDate) {
		String fromDateStr = localDateTimeUtility.getDateTimeByFormat(fromDate, IntxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String toDateStr = localDateTimeUtility.getDateTimeByFormat(toDate, IntxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String query = LoadJpaQueries.getQueryById(IntxConstant.GET_T_AWAY_TRANSACTION_DUPLICATE_CHECK_FRM_FPMS);
		List<AwayTransaction> list = jdbcTemplate.query(query,
				new BeanPropertyRowMapper<AwayTransaction>(AwayTransaction.class), deviceNo, plazaId, laneId,
				fromDateStr, toDateStr);
	...
	@Override
	public void insertIntxInAgencyPending(AwayTransaction awayTransaction) {
		String query = LoadJpaQueries.getQueryById(IntxConstant.INSERT_INTO_T_AGENCY_TX_PENDING);
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				...
				setFullFareAmount(awayTransaction, ps);
				setDiscountedAmount(awayTransaction, ps);
				setDstAtpFileId(awayTransaction, ps);
				return ps;
			}
		};
	...
	private void setDstAtpFileId(AwayTransaction awayTransaction, final PreparedStatement ps) throws SQLException {
		if (awayTransaction.getAtpFileId() != null) {
			ps.setLong(97, awayTransaction.getAtpFileId());
		} else {
			ps.setNull(97, java.sql.Types.NUMERIC);
		}
	}
	
	private void setDiscountedAmount(AwayTransaction awayTransaction, final PreparedStatement ps) throws SQLException {
		if (awayTransaction.getDiscountedAmount() != null) {
			ps.setDouble(96, awayTransaction.getDiscountedAmount());
		} else {
			ps.setNull(96, java.sql.Types.NUMERIC);
		}
	}
	
	private void setFullFareAmount(AwayTransaction awayTransaction, final PreparedStatement ps) throws SQLException {
		if (awayTransaction.getFullFareAmount() != null) {
			ps.setDouble(95, awayTransaction.getFullFareAmount());
		} else {
			ps.setNull(95, java.sql.Types.NUMERIC);
		}
	}
...
//		}
	...
	private void setTxDate(AwayTransaction awayTransaction, final PreparedStatement ps) throws SQLException {
		if (awayTransaction.getTxDate()!=null) {
	...
	private void setCorrReasonId(AwayTransaction awayTransaction, final PreparedStatement ps) throws SQLException {
		if (awayTransaction.getEtcCorrReasonId() != null) {
			ps.setString(75, awayTransaction.getEtcCorrReasonId());
		} else {
...
//		}
		if (!StringUtils.isBlank(awayTransaction.getEtcTagPgmStatus())) {
			ps.setString(59, awayTransaction.getEtcTagPgmStatus());
		} else {
			ps.setNull(59, java.sql.Types.VARCHAR);
		}
	}
...
//		}
		if (awayTransaction.getEntryTxSeqNumber() != null) {
			ps.setLong(3, awayTransaction.getEntryTxSeqNumber());
		} else {
			ps.setNull(3, java.sql.Types.NUMERIC);
		}
	}
	...
	@Override
	public void deleteFromIntxLogs(AwayTransaction awayTransaction) {
		logger.info("Delete record from T_EPS_INTX_LOG...");

		String query = LoadJpaQueries.getQueryById(IntxConstant.DELETE_FROM_T_EPS_INTX_LOG);
		...
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				final PreparedStatement ps = connection.prepareStatement(query);
				//ps.setLong(1, awayTransaction.getLaneTxId());
				ps.setString(1, awayTransaction.getEtcTagAgency());
				ps.setString(2, String.valueOf(awayTransaction.getExternFileId()));
				ps.setString(3, String.valueOf(awayTransaction.getEtcTrxSerialNum()));
				return ps;
			}
		};

		jdbcTemplate.update(psc);
		logger.info("Deleted record from T_EPS_INTX_LOG.");
	}
	
	@Override
	public void deleteFromItxnCorrectionLogs(AwayTransaction awayTransaction) {
		logger.info("Delete record from T_EPS_ITXN_CORRECTION_LOG...");

		String query = LoadJpaQueries.getQueryById(IntxConstant.DELETE_FROM_T_EPS_ITXN_CORRECTION_LOG);
		...
		PreparedStatementCreator psc = new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				final PreparedStatement ps = connection.prepareStatement(query);
				ps.setLong(1, awayTransaction.getLaneTxId());
				ps.setString(2, awayTransaction.getEtcTagAgency());
				ps.setString(3, String.valueOf(awayTransaction.getExternFileId()));
				ps.setString(4, awayTransaction.getEtcCorrReasonId());
				return ps;
			}
		};

		jdbcTemplate.update(psc);
		logger.info("Deleted record from T_EPS_ITXN_CORRECTION_LOG.");
	}
