...
public class AwayTransactionDaoImpl implements AwayTransactionDao {
	 ...
	 */
	@Override
	public List<AwayTransaction> getAwayTransactionByAgencyId(Long awayAgencyId) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IctxConstant.GET_T_AWAY_TRANSACTION);
		paramSource.addValue(IctxConstant.ACCOUNT_AGENCY_ID, awayAgencyId + "%");
	...
	public List<AwayTransaction> getCorrAwayTransactionByDevicePrefix(String devicePrefix) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IctxConstant.GET_T_CORR_AWAY_TRANSACTION);
		paramSource.addValue(IctxConstant.DEVICE_PREFIX, devicePrefix + "%");
		//System.out.println("queryFile "+ queryFile);
		List<AwayTransaction> tempAwayTxList = namedParameterJdbcTemplate.query(queryFile, paramSource,
				BeanPropertyRowMapper.newInstance(AwayTransaction.class));
		if (tempAwayTxList != null) {
	...
	public void updateDstAtpFileId(List<AwayTransaction> awayTransactionList, Long atpFileId) {
		try {
			...
			String query = LoadJpaQueries.getQueryById(IctxConstant.UPDATE_DST_ATP_FILE_ID);
			jdbcTemplate.batchUpdate(query, new BatchPreparedStatementSetter() {
				@Override
				public void setValues(PreparedStatement ps, int i) throws SQLException {
					AwayTransaction awayTransaction = awayTransactionList.get(i);
					ps.setLong(1, atpFileId);
					ps.setLong(2, IctxConstant.TX_STATUS_QICTX);
					ps.setLong(3, awayTransaction.getLaneTxId());
				}
	...
	public void updateExpectedRevenueAmtAndRevenueDate(AwayTransaction awayTransaction) {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IctxConstant.UPDATE_EXPECTED_REVENUE_AMOUNT_AND_REVENUE_DATE);
		paramSource.addValue(IctxConstant.QUERY_PARAM_EXPECTED_REVENUE_AMOUNT, awayTransaction.getExpectedRevenueAmount());
		paramSource.addValue(IctxConstant.QUERY_PARAM_REVENUE_DATE, awayTransaction.getRevenueDate());
		paramSource.addValue(IctxConstant.QUERY_PARAM_LANE_TX_ID, awayTransaction.getLaneTxId());
		namedParameterJdbcTemplate.update(queryFile, paramSource);
	}

	
	/**
	 * Update Posted fare amount and discounted amount
	 * 
	 * @param awayTransaction
	 */
	@Override
	public void updatePostedFareAmount(AwayTransaction awayTransaction, Double etcTollAmount) {
		logger.info("Updating awayTransaction.getLaneTxId() {} :: etcTollAmount {}",awayTransaction.getLaneTxId(),etcTollAmount);
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IctxConstant.UPDATE_POSTED_TOLL_AMOUNT);
		paramSource.addValue(IctxConstant.POSTED_FARE_AMOUNT, etcTollAmount);
		paramSource.addValue(IctxConstant.DISCOUNTED_AMOUNT, etcTollAmount);
		paramSource.addValue(IctxConstant.FULL_FARE_AMOUNT, awayTransaction.getFullFareAmount());
		paramSource.addValue(IctxConstant.QUERY_PARAM_LANE_TX_ID, awayTransaction.getLaneTxId());
		namedParameterJdbcTemplate.update(queryFile, paramSource);
	}
	...
	public Integer deleteRejectedTx(Long laneTxId, String fileType) {
		...
		query = LoadJpaQueries.getQueryById(IctxConstant.DELETE_REJECTED_TX);
	 ...
	 */
	@Override
	public Integer deleteRejectedTxNew(Long laneTxId, String fileType, String txExternRefNo) {
		
		logger.info("Updating awayTransaction.laneTxId() {} :: txExternRefNo {}",laneTxId, txExternRefNo);
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IctxConstant.DELETE_REJECTED_TX_NEW);
		paramSource.addValue(IctxConstant.QUERY_PARAM_LANE_TX_ID, laneTxId);
		paramSource.addValue(IctxConstant.TX_EXTERN_REF_NO, txExternRefNo);
		return namedParameterJdbcTemplate.update(queryFile, paramSource);
	 ...
	 */
	@Override
	public Integer deleteRejectedITXCQ(Long laneTxId, String fileType) {
		
		String query = "";
		
		query = LoadJpaQueries.getQueryById(IctxConstant.DELETE_REJECTED_CORR);
	...
	public Optional<List<AwayTransaction>> getAwayTransaction(String deviceNo, Integer plazaId, Integer laneId,
		...
		String fromDateStr = localDateTimeUtility.getDateTimeByFormat(fromDate, IctxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String toDateStr = localDateTimeUtility.getDateTimeByFormat(toDate, IctxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String query = LoadJpaQueries.getQueryById(IctxConstant.GET_T_AWAY_TRANSACTION_DUPLICATE_CHECK);
	...
	@Override
	public Optional<List<AwayTransaction>> getAwayTransactionNew(Long laneTxId) {
		String query = LoadJpaQueries.getQueryById(IctxConstant.GET_T_AWAY_TRANSACTION_DUPLICATE_CHECK_NEW);
		List<AwayTransaction> list = jdbcTemplate.query(query,
				new BeanPropertyRowMapper<AwayTransaction>(AwayTransaction.class), laneTxId);
	...
	public Optional<List<AwayTransaction>> getAwayTransactionFpms(String deviceNo, Integer plazaId, Integer laneId,
		...
		String fromDateStr = localDateTimeUtility.getDateTimeByFormat(fromDate, IctxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String toDateStr = localDateTimeUtility.getDateTimeByFormat(toDate, IctxConstant.FORMAT_yyyyMMddHHmmssSSS);
		String query = LoadJpaQueries.getQueryById("GET_T_AWAY_TRANSACTION_DUPLICATE_CHECK_FRM_FPMS");
		List<AwayTransaction> list = jdbcTemplate.query(query,
				new BeanPropertyRowMapper<AwayTransaction>(AwayTransaction.class), deviceNo, plazaId, laneId,
				fromDateStr, toDateStr);
	...
	@Override
	public void insertITXCInAgencyPending(AwayTransaction awayTransaction) {
	...
	private void setTxStatus(AwayTransaction awayTransaction, final PreparedStatement ps) throws SQLException {
		if (awayTransaction.getTxStatus() != null) {
			ps.setInt(94, awayTransaction.getTxStatus());
		} else {
...
//			ps.setTimestamp(8, Timestamp.valueOf(awayTransaction.getTxTimestamp()));
	...
	@Override
	public List<AwayTransaction> getCorrAwayTransactionHub() {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IctxConstant.GET_T_CORR_AWAY_TRANSACTION_ALL_AGENCY);
		//System.out.println("queryFile "+ queryFile);
		List<AwayTransaction> tempAwayTxList = namedParameterJdbcTemplate.query(queryFile, paramSource,
				BeanPropertyRowMapper.newInstance(AwayTransaction.class));
		if (tempAwayTxList != null) {
	...
	@Override
	public List<AwayTransaction> getAwayTransactionHub() {
		MapSqlParameterSource paramSource = new MapSqlParameterSource();
		String queryFile = LoadJpaQueries.getQueryById(IctxConstant.GET_T_AWAY_TRANSACTION_ALL_AGENCY);
		//System.out.println("queryFile "+ queryFile);
		List<AwayTransaction> tempAwayTxList = namedParameterJdbcTemplate.query(queryFile, paramSource,
				BeanPropertyRowMapper.newInstance(AwayTransaction.class));
		if (tempAwayTxList != null) {
