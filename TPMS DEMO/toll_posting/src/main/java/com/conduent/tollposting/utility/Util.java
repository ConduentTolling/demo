...
public class Util 
{
	...
	public static void setRevenueDate(AccountTollPostDTO tollPost, MasterCache masterCache)
	{
		try
		{
			...
			tollPost.setRevenueDate(tollPost.getRevenueDate()); 
			int plazaRevenueTimeSec = tempRevenueDateStatisticsDto.getRevenueSecondOfDay();
			
			if(tollPost.getPlazaAgencyId() != 3 && tollPost.getPlazaAgencyId() != 19 && tollPost.getPlazaAgencyId() != 63) {		//added condition for PB & TIBA
