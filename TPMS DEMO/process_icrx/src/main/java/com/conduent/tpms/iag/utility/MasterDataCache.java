...
import com.conduent.tpms.iag.dto.RevenueDateStatisticsDto;
...
*/	
	...
	public SystemAccountVO getParamValue(Integer paramConfig) {
		...
		Optional<SystemAccountVO> plan = systemAccountList.stream().filter(e -> e.getParamConfig().equals(paramConfig)).findFirst();
		if (plan.isPresent()) {
	...
	public SystemAccountVO getParamValueByAgencyId(Integer agencyId) {
		if (agencyId == null || systemAccountList == null) {
			log.info("Param config value is null or not present..");
			return null;
		}
		
		Optional<SystemAccountVO> plan = systemAccountList.stream().filter(e -> e.getAgencyId().equals(agencyId)).findFirst();
		if (plan.isPresent()) {
			log.info("Param config value is presennt get systemAccount object ==> "+plan.toString());
			return plan.get();
		}
	...
	public AgencyInfoVO getAccountAgencyByAgencyId(Integer agencyId) {
		if (agencyId == null || agencyInfoVOList == null) {
			log.info("fromAgencytId value is null or not present");
			return null;
		}
		Optional<AgencyInfoVO> agencyInfo = agencyInfoVOList.stream().filter(e -> e.getAgencyId().equals(agencyId)).findFirst();
		if (agencyInfo.isPresent()) {
			log.info("agencyId value present inside getAccountAgencyId method"+agencyInfo.toString());
			return agencyInfo.get();
		}
		return null;
	}
