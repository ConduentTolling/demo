...
public class TAgencyDaoImpl implements TAgencyDao {
	...
	@Override
	public List<String> getAllHomeAgencies() {

		List<String> info = null;
		try 
		{
			dao_log.info("Getting list of home agencies from table..");
			
			String queryFile = LoadJpaQueries.getQueryById("GET_ALL_HOME_AGENCIES");

			info = jdbcTemplate.queryForList(queryFile,String.class);
		}
		catch(Exception e) 
		{
			e.getMessage();
