...
public class SummaryFileCreationServiceImpl extends SummaryFileCreationService{
	...
	public void buildSummaryFile(Map<String,String> recordCountMap, String inputFileType, String destPath,String fileNAme) {
		...
		String T_INCR_TAG_STATUS_ALLSORTED = recordCountMap.get(Constants.T_INCR_TAG_STATUS_ALLSORTED)==null?Constants.DEFAULT_COUNT:recordCountMap.get(Constants.T_INCR_TAG_STATUS_ALLSORTED);
		...
		try (FileWriter filewriterSummaryNew = new FileWriter(file, true)) {
			  ...
			  content.append("Statistics of IFGU file "+fileNAme+" created on : "+timeZoneConv.currentTime().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))+" "+timeZoneConv.currentTime().format(DateTimeFormatter.ofPattern("hh:mm:ss")));
			  filewriterSummaryNew.write(content.toString());
			  filewriterSummaryNew.write("\r\n");
			  filewriterSummaryNew.write("---------------------------------------------------------------------------------------- \r\n");
			  filewriterSummaryNew.write("\r\n");
			  filewriterSummaryNew.write(T_INCR_TAG_STATUS_ALLSORTED+" Total tags in the T_INCR_TAG_STATUS_ALLSORTED\r\n");
			  filewriterSummaryNew.write("\r\n");
			  filewriterSummaryNew.write(TOTAL+" IFGU tags were read from the database.\r\n");
			  ...
			  filewriterSummaryNew.write("\r\n");
			  filewriterSummaryNew.write(TOTAL+" total IFGU tags.\r\n");
			  log.info("Content appended to file {}", file.getName());
			  log.info("Records counts are T_INCR_TAG_STATUS_ALLSORTED:{}, TOTAL:{}, LOST:{}, INVALID:{}, LOW_BAL:{}, GOOD:{}, SKIP:{}",T_INCR_TAG_STATUS_ALLSORTED, TOTAL, LOST, INVALID, LOW_BAL, GOOD, SKIP);
