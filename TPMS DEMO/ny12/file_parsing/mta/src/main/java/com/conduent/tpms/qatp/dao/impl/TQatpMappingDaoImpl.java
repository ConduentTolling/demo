...
public class TQatpMappingDaoImpl implements TQatpMappingDao {
	...
	public FileParserParameters getMappingConfigurationDetails(FileParserParameters fielDto) {
		...
		List<MappingInfoDto> laneHeaderMappingDetails = nameMappingDetails.stream()
				.filter(f -> f.getFieldType().equals("LANEHEADER")).collect(Collectors.toList());

		fileDetails.setLaneHeaderappingInfo(laneHeaderMappingDetails);
	...
	public XferControl checkFileDownloaded(String fileName) {
		...
		String queryToCheckFile = "SELECT XFER_CONTROL_ID, XFER_FILE_ID, XFER_FILE_NAME, DATE_CREATED, TIME_CREATED, FILE_SIZE, FILE_CHECKSUM, NUM_RECS, HASH_TOTAL, XFER_PATH, XFER_XMIT_STATUS, XFER_RETRY, UPDATE_TS, FILE_TYPE  FROM MASTER.T_XFER_CONTROL WHERE XFER_FILE_NAME='"
				+ fileName + "' AND XFER_XMIT_STATUS ='C' order by XFER_CONTROL_ID desc";// LoadJpaQueries.getQueryById("CHECK_FILE_DOWNLOADED");
		// paramSource.addValue(QATPConstants.XFER_FILE_NAME,
		// FilenameUtils.getBaseName(fileName));
		// paramSource.addValue(QATPConstants.XFER_XMIT_STATUS,
		// QATPConstants.PROCESS_STATUS_COMPLETE);
		// XferControl obj = namedJdbcTemplate.queryForObject(queryToCheckFile,
		// paramSource,BeanPropertyRowMapper.newInstance(XferControl.class));
