	...
	public long publishMessages(List<TranDetail> txlist) 
	{
		...
		for (int i = 0; i < txlist.size(); i++) 
		{
			...
			if(tempTx.getPlazaAgencyId()==null) {
				tempTx.setPlazaAgencyId(Convertor.toInteger(QATPConstants.MTA_AGENCY_ID));
			}
			if(tempTx.getPlazaId()==null) {
				tempTx.setPlazaId(masterDataCache.getPlazaIfNull(Convertor.toInteger(QATPConstants.MTA_AGENCY_ID)));
			}
