...
public class TQatpMappingDaoImpl implements TQatpMappingDao 
{
	...
	public FileParserParameters getMappingConfigurationDetails1(FileParserParameters fielDto) {
		...
		/*
		 * String queryFile =
		 * LoadJpaQueries.getQueryById("GET_FILE_INFO_CONFIGURATION");
		 * 
		 * paramSource.addValue(Constants.FILE_TYPE, fielDto.getFileType());
		 * paramSource.addValue(Constants.AGENCY_ID, fielDto.getAgencyId());
		 * paramSource.addValue(Constants.FILE_FORMAT, Constants.FIXED);
		...
		//paramSource.addValue(Constants.AGENCY_ID, 5);
		String queryRules = LoadJpaQueries.getQueryById("GET_API_VALIDATION");
		...
		/*
		 * List<MappingInfoDto>
		 * fileNameMappingDetails=nameMappingDetails.stream().filter(f->
		 * f.getFieldType().equals("FILENAME")).collect(Collectors.toList());
		 * 
		 * fileDetails.setFileNameMappingInfo(fileNameMappingDetails);
		 * fileDetails.setFileNameSize(fileNameMappingDetails.get(fileNameMappingDetails
		 * .size()-1).getEndPosition());
		 */

		/*
		 * List<MappingInfoDto>
		 * headerMappingDetails=nameMappingDetails.stream().filter(f->
		 * f.getFieldType().equals("HEADER")).collect(Collectors.toList());
		 * 
		 * fileDetails.setHeaderMappingInfoList(headerMappingDetails);
		 * 
		 * fileDetails.setHeaderSize(headerMappingDetails.get(headerMappingDetails.size(
		 * )-1).getEndPosition());
		 */
		...
		/*
		 * List<MappingInfoDto> trailerMappingDetails
		 * =nameMappingDetails.stream().filter(f->
		 * f.getFieldType().equals("TRAILER")).collect(Collectors.toList());
		 * fileDetails.setTrailerMappingInfoDto(trailerMappingDetails);
		 * 
		 * fileDetails.setTrailerSize(trailerMappingDetails.get(trailerMappingDetails.
		 * size()-1).getEndPosition());
		 */
